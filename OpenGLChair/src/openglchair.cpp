//Make into a packaged app:
//https://stackoverflow.com/questions/5131015/creating-an-application-bundle-on-osx-with-eclipse-c

// Linker options -lglew -lglfw -framework Cocoa -framework OpenGL -framework IOKit -framework CoreVideo
...
#include "SOIL2/SOIL2.h" // Include SOIL2 texture loader (put in src folder)
...
// Object file and texture files to load
// Important: These need to be in the project folder
const char* ObjectFile   = "chair_object.obj";
...
const char* TextureFile5 = "sky_texture.jpg";

// Settings
GLuint WINDOW_WIDTH = 1200, WINDOW_HEIGHT = 800;
GLuint SHADOW_WIDTH = 1024, SHADOW_HEIGHT = 1024;
GLuint VBO[4], VAO[4], texture[6];
GLuint specularShader, diffuseShader, lampShader, depthShader, debugDepthShader;
GLuint depthMapFBO;
GLfloat near_plane = 0.01f, far_plane = 15.0f; // Shadow extents
GLfloat cameraDistance = 5;
GLfloat cameraRotV = 75;
GLfloat cameraRotH = 45;
GLfloat mousex, mousey;
GLboolean rotateObject = false;
GLboolean moveCamera   = false;
GLboolean keyModifier  = false;
GLboolean perspective  = true;

// Camera parameters
glm::vec3 cameraPosition = glm::vec3(0.0f, 1.0f, 2.5f);
glm::vec3 cameraUpY = glm::vec3(0.0f, 1.0f, 0.0f);
glm::vec3 cameraTarget = glm::vec3(0.0f, 0.0f, 0.0f);

// Light parameters
glm::vec3 lightColor = glm::vec3(1.0f, 1.0f, 1.0f);
glm::vec3 lightPosition = glm::vec3(-2.5f, 2.5f, 0.0f);
...
const GLchar * specularFragmentShader = GLSL(330,
	...
	void main(){
	    ...
	    // get diffuse color
	    vec4 color = texture(uTexture, mobileTextureCoordinate);

	     // obtain normal from normal map in range [0,1]
	    vec3 normal = texture(uNormal, mobileTextureCoordinate).rgb;
	    ...
	    objColor = attenuation * vec4(ambient + (1.0 - shadow) * (diffuse + specular), 1.0) * color;
	}
);
...
/* Debug depth vertex shader source */
const GLchar * debugVertexShader = GLSL(330,
	...
	void main()
	{
		mobileTextureCoordinate = textureCoordinates;
...
const GLchar * debugFragmentShader = GLSL(330,
	...
	void main()
	{
		float depthValue = texture(depthMap, mobileTextureCoordinate).r;
		FragColor = vec4(vec3(LinearizeDepth(depthValue) / far_plane), 1.0); // perspective (for point light)
		//FragColor = vec4(vec3(depthValue), 1.0);                           // orthographic (for distant light)
	}
);
...
void CreateBuffers() {
	...
	loadOBJ(ObjectFile); // Load chair_object.obj file data into vectors
	...
	for (int i = 0; i < (int)objVertices.size(); ++i) {
		...
		vertices[14*i +  8] = objTangents.at(i).x;   // Tangents
		vertices[14*i +  9] = objTangents.at(i).y;
		vertices[14*i + 10] = objTangents.at(i).z;
		vertices[14*i + 11] = objBitangents.at(i).x; // Bitangents
	...
	// Set attribute pointer 2 to hold UV data
...
void RenderGraphics(void) {
	...
	// Elapsed time used to orbit around object
	GLfloat time = 0.5 * glfwGetTime();

	// Transform the light
	lightPosition[0] = 3.0f * cos(time);
	lightPosition[1] = 2.0f;
	lightPosition[2] = 3.0f * sin(time);
...
void generateTexture() {
    ...
    glGenTextures(6, texture); // Create 3 textures

    int width, height; // Texture dimensions
    unsigned char* image; // Reference to image
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR); // Define texture filtering
	//glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	SOIL_free_image_data(image); // Deactivate image after using

    // Chair normal
    glActiveTexture(GL_TEXTURE1);
	glBindTexture(GL_TEXTURE_2D, texture[1]); // Bind chair texture
	image = SOIL_load_image(TextureFile2, &width, &height, 0, SOIL_LOAD_RGB); // Load chair texture
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, image); // Define texture
	glGenerateMipmap(GL_TEXTURE_2D); // Generates mipmaps for the specified texture object
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR); // Define texture filtering
	//glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	...
	glBindTexture(GL_TEXTURE_2D, texture[2]); // Bind chair texture
	image = SOIL_load_image(TextureFile3, &width, &height, 0, SOIL_LOAD_RGB); // Load chair texture
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, image); // Define texture
	glGenerateMipmap(GL_TEXTURE_2D); // Generates mipmaps for the specified texture object
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR); // Define texture filtering
	//glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	...
	glBindTexture(GL_TEXTURE_2D, texture[3]); // Bind chair texture
	image = SOIL_load_image(TextureFile4, &width, &height, 0, SOIL_LOAD_RGB); // Load chair texture
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, image); // Define texture
	glGenerateMipmap(GL_TEXTURE_2D); // Generates mipmaps for the specified texture object
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR); // Define texture filtering
	//glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
...
int main()
{
    ...
    // --------------------
    GLFWwindow* window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "OpenGL Demonstration", NULL, NULL);
    if (window == NULL)
    {
...
void framebuffer_size_callback(GLFWwindow* window, int w, int h)
{
    ...
    glViewport(0, 0, WINDOW_WIDTH=w, WINDOW_HEIGHT=h);
}
...
void mouse_button_callback(GLFWwindow* window, int button, int action, int mods)
{
	...
	if (action == GLFW_PRESS) {
		...
		case GLFW_MOUSE_BUTTON_LEFT:
			rotateObject = GL_TRUE; // Set rotateObject flag to true
			break;

		// If right mouse button is pressed
		case GLFW_MOUSE_BUTTON_RIGHT:
			moveCamera = GL_TRUE; // Set moveCamera flag to true
			break;
	...
	// If mouse button is released
		...
		case GLFW_MOUSE_BUTTON_LEFT:
			rotateObject = GL_FALSE; // Set rotateObject flag to false
			break;

		// If right mouse button is released
		case GLFW_MOUSE_BUTTON_RIGHT:
			moveCamera = GL_FALSE; // Set moveCamera flag to false
			break;
...
void cursor_position_callback(GLFWwindow* window, double x, double y) {
	...
	if (rotateObject) {
		GLfloat dy = (y - mousey) * 360.0 / WINDOW_WIDTH;
		GLfloat dx = (x - mousex) * 360.0 / WINDOW_HEIGHT;
...
void scroll_callback(GLFWwindow* window, double xoffset, double yoffset)
{
    cameraDistance += yoffset*0.1;
    cameraDistance = (cameraDistance < 0) ? 0.001 : cameraDistance;
}


// Simple method to load an obj file
// Object must consist of only triangular polygons and have normals and UV coordinates
...
void loadOBJ(const char* path)
{
	...
	std::cout<<"Polygons: "<<facecount<<std::endl;
